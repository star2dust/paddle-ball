...
class Paddle():
    ...
    def reset(self):
        ...
        self.paddle.goto(0, -275)
        self.ball.goto(0, 100)
        # random velocity after reset
        self.ball.dx *= np.random.choice((-1, 1))
    ...
    def run_frame(self):
        ...
        if self.ball.ycor() < -290:
            self.ball.goto(0, 100)
            # random velocity after hit the ground
            self.ball.dx *= np.random.choice((-1, 1))
            self.miss += 1
            self.score.clear()
            self.score.write("Hit: {}   Missed: {}".format(self.hit, self.miss), align='center', font=('Courier', 24, 'normal'))
            self.reward -= 3
            # hit center is better
            #self.reward -= min(0.5, abs(round(abs(self.paddle.xcor() - self.ball.xcor())/55-1,1)))
            self.done = True
        ...
        if abs(self.ball.ycor() + 250) < 2 and abs(self.paddle.xcor() - self.ball.xcor()) < 55:
            ...
            self.reward += 3
            # hit center is better
            #self.reward += min(0.5, round(1-abs(self.paddle.xcor() - self.ball.xcor())/55,1))
